...
$contents.="<form action='javascript:void(0)' method='POST' name='openinviter'>".ers($ers).oks($oks);
if (!$done)
	{
	if ($step=='get_contacts')
		{
		...
		$contents.="<table class='thTable' cellspacing='0' cellpadding='0' style='border:none;'>
				<tr><td align='center' valign='top'><label for='message_box'>Message</label></td><td><textarea id='autosend' rows='5' cols='50' name='message_box' style='width:300px;'>{$_POST['message_box']}</textarea></td></tr>
	...
	if ($step=='send_invites')
		...
		if ($inviter->showContacts())
			...
			if (count($contacts)==0)
				...
				$contents.="<tr><td colspan='".($plugType=='email'? "3":"2")."' style='padding:3px;'><input type='submit' id='import' name='send' value='Send invites'></td></tr>";
				}
			$contents.="</table>";
			}
