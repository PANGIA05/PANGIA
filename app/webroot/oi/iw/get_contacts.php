...
//alert('okoka singh');
	    var elemArray = document.getElementsByName('emails');
	            var abc = '';
    ...
    parent.incomingValue(names1);
  //  $('#result').append('hello admin');
  // localStorage.setItem('movie' + names1);
...
$contents.="<form action='' id='mainform' method='POST' name='openinviter'>".ers($ers).oks($oks);
if (!$done)
	{
	if ($step=='get_contacts')
		{
		$contents.="<table align='center' class='thTable' cellspacing='2' cellpadding='0' style='border:none;'>
			...
			<tr><td colspan='2' align='center'><input type='submit' name='import' id='invites' value='Import Contacts'></td></tr>
		...
		if ($inviter->showContacts())
			...
			if (count($contacts)==0)
				...
				foreach ($contacts as $email=>$name)
					{
					$counter++;					
					$contents.="<tr><td><input name='check_{$counter}' value='{$counter}' type='checkbox' checked><input type='hidden' name='email_{$counter}' value='{$email}'>
					<input type='hidden'  id='name{$counter}'  name='emails' value='{$email}'>
					
					<input type='hidden' name='name_{$counter}' value='{$name}'></td>".($plugType == 'email' ?"<td>{$email}</td>":"")."</tr>";
					}
				$contents.="<tr><td colspan='".($plugType=='email'? "3":"2")."' style='padding:3px;'><input type='submit' id='imports'  name='send' value='Send invites'></td></tr>";
				}
			$contents.="</table>";
			}
